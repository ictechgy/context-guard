...
`claude-token-bench`는 `research/benchmark-plan.md` 실행을 자동화합니다. JSON fixture에서 task와 variant 정의를 읽어 각 조합에 대해 `claude -p --output-format json`을 호출하고, fixture 의 `success_command` 를 실행한 뒤 `tokens_per_successful_task` 측정용 CSV에 한 행을 append 합니다. `--ledger-jsonl` 을 추가하면 run별 cost-shift ledger를 남기고, `--report-json` 을 추가하면 baseline 대비 A/B report를 생성해 실제 token/cost 절감과 proxy byte 감소를 분리해 보여줍니다. `--dry-run` 은 실제 호출 없이 어떤 명령이 실행될지만 보여주고, `--resume` 은 CSV 에 이미 적재된 `(task_id, variant)` 쌍을 건너뜁니다. `success_command` 는 `shlex.split + shell=False` 로 실행되므로 fixture JSON 자체는 shell-injection 표면이 되지 않습니다 — 파이프·리디렉션이 필요한 검증은 별도 헬퍼 스크립트로 분리하고 그 경로를 `success_command` 로 둡니다.
