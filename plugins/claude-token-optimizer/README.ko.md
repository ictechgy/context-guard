...
가장 먼저 `/claude-token-optimizer:setup`을 실행하세요. 이 명령은 프로젝트 로컬 설정만 적용하고, 적용 후 남은 gap을 read-only diet scan 요약으로 보여줍니다. 전역 Claude 설정은 변경하지 않으며, 외부 AI 위임/offload도 설정하지 않습니다.
...
이 helper들은 흔한 토큰 낭비 원인을 줄이지만 고정 절감률을 보장하지는 않습니다. 실제 before/after 증거가 필요하면 `claude-token-bench --ledger-jsonl ... --report-json ...`로 본인 작업에서 측정하세요.
...
마법사는 `.claude/settings.json`을 덮어쓰지 않고 병합합니다. 외부 AI 위임 설정은 더 이상 제공하지 않습니다. 설정을 적용하면 read-only `claude-token-diet scan` 요약을 자동으로 출력해 남은 gap을 확인합니다. 자동화에서 post-apply scan 요약이 불필요하면 `--no-diet-scan`을 사용하세요.
...
`claude-token-failed-nudge`는 같은 Bash 명령이 같은 세션에서 연속 두 번 실패하면 `/clear` (또는 `/compact focus on …`)을 권유하는 `PostToolUse` hook 입니다. 세 번째 반복 실패부터는 strategy-switch signal을 추가해 동일 명령 경로 재시도 대신 다른 가설, 더 작은 재현, 다른 진단 범위로 전환하게 합니다. 실패 시도가 누적되면 대화 컨텍스트가 오염되고 prompt cache 가 매 retry 마다 재워밍되어 토큰 비용이 급증합니다. 본 hook 은 짧은 추가 컨텍스트만 주입해 방향 전환을 유도합니다 (실행은 막지 않습니다). 반복 실패 이후에만 발화하고 실행을 막지 않기 때문에 recommended project-local setup에서는 기본으로 켜집니다. 프로젝트에서 힌트가 시끄러우면 `claude-token-setup --no-failed-attempt-nudge` (또는 대화형 마법사의 "no")로 제외하세요. 상태는 프로젝트 로컬 `.claude-token-optimizer/failures-<session>.json` (파일 모드 `0o600`)에 저장됩니다.
