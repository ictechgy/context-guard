...
def without_location_prefix(compiled: re.Pattern[str]) -> re.Pattern[str]:
    ...
    return re.compile(source.replace(LOCATION_PREFIX_FRAGMENT, "", 1))
...
def redact_secret_assignments(
    ...
    *,
    ...
    line = INLINE_QUOTED_SECRET_ASSIGNMENT_NO_LOCATION_RE.sub(quoted_repl, line)
    line = INLINE_UNQUOTED_FALLBACK_SECRET_ASSIGNMENT_NO_LOCATION_RE.sub(unquoted_repl, line)
    line = INLINE_UNQUOTED_CALL_SECRET_ASSIGNMENT_NO_LOCATION_RE.sub(unquoted_repl, line)
    line = INLINE_UNQUOTED_BRACKETED_SECRET_ASSIGNMENT_NO_LOCATION_RE.sub(unquoted_repl, line)
    line = INLINE_UNQUOTED_SECRET_ASSIGNMENT_NO_LOCATION_RE.sub(unquoted_repl, line)
...
def detect_multiline_secret_assignment(line: str) -> str | None:
    ...
    """
    line = scan_location_prefix(line).remainder
    for marker in MULTILINE_SECRET_ASSIGNMENT_NO_LOCATION_RE.finditer(line):
...
def detect_multiline_secret_expression(line: str) -> int | None:
    ...
    line = scan_location_prefix(line).remainder
    marker = UNQUOTED_MULTILINE_SECRET_ASSIGNMENT_NO_LOCATION_RE.search(line)
    if marker is None:
        return None
...
class LineSanitizer:
    ...
    def sanitize(self, raw_line: str) -> tuple[str, bool]:
        ...
        new_body, count = AUTH_HEADER_NO_LOCATION_RE.subn(
        ...
        new_body, count = COOKIE_HEADER_NO_LOCATION_RE.subn(
            r"\g<prefix>[REDACTED]", header_body
        )
        if count:
            redacted = True
