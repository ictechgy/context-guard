...
- **Artifact store** saves large sanitized command output under `.context-guard/artifacts` by default and returns compact receipts or exact requested slices. JSON receipts include line-numbered top errors, duplicate-line groups, and exact suggested queries. `get` and `list` can also read legacy `.claude-token-optimizer/artifacts` receipts.
