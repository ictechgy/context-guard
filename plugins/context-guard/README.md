...
`plan image-context-pack` is plan-only and dry-run: it does not render or parse images, run OCR, emit replacements, write files, store binary image artifacts, call providers, or proxy traffic. It records explicit opt-in, protected-zone denial, missed-context notes, exact text fallback requirements, provider-measured boundaries, and its relation to `visual-crop-ocr`, which remains caller-supplied visual evidence rather than verified exact binary fallback.
