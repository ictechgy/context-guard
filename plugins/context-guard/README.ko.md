...
- **로컬 아티팩트 보관소**는 큰 명령 출력을 기본적으로 `.context-guard/artifacts`에 정제해 저장하고, 줄 번호가 있는 top error, 중복 라인 그룹, 정확한 suggested query가 담긴 요약 영수증이나 요청한 정확한 줄 범위만 반환합니다. `get`과 `list`는 리브랜딩 이전의 `.claude-token-optimizer/artifacts` 영수증도 읽을 수 있습니다.
