...
| Experimental planners and local runtimes | Reviewing local-proxy ideas with a dry-run plan plus explicit local proxy gate records, alongside explicit local context-diff, visual evidence-pack, learned-compression candidate, and self-hosted metrics runtimes. | Default-off and explicit-command-only; local proxy gate rows start no listener and forward no traffic; no compressor/model execution, generated compression, OCR/crop service, actual forwarding, or hosted-savings claim without separate evidence and future PR gates. |
...
- Experimental helpers are mostly dry-run checker/planner surfaces, with explicit local runtimes only for caller-supplied context-diff replacement payloads, caller-supplied visual crop/OCR evidence packs, caller-supplied learned-compression prose candidates, self-hosted metrics JSONL sidecar records, and local-proxy runtime-gate JSONL records. ContextGuard can record local-proxy gate readiness, but it does not ship learned/synthetic compressor execution, embeddings, rerankers, model calls, generated replacement text, screenshot capture, image cropping, OCR execution, image parsing, external OCR/image services, self-hosted KV/latent inference optimization beyond explicit local metrics recording, or actual proxy forwarding runtime.
...
The `plan local-proxy` example still produces advisory metadata only; it does not enable forwarding. The explicit `record local-proxy-runtime-gate` example appends one localhost-only gate row and still starts no listener, forwards no traffic, persists no API keys, and makes no hosted-savings claim.
...
| `local-proxy` | Localhost-only advisory metadata plus an explicit `record local-proxy-runtime-gate --ledger-jsonl` runtime for one local gate row. | `plan` writes no ledger. `record` writes only after localhost-only metadata and `--runtime-gate-ack`; it starts no listener, forwards no traffic, performs no DNS lookup, persists no API keys, calls no external services, blocks non-local bind/target/upstream values, and requires a separate future runtime gate before any forwarding implementation. |
...
- Learned/synthetic compressor execution or generated replacement text beyond the caller-supplied learned candidate emitter, generated crop/OCR or visual-token pruning runtime beyond the caller-supplied visual evidence-pack emitter, self-hosted KV/latent optimization beyond explicit local metrics recording, and actual proxy forwarding runtime beyond the local proxy gate recorder. See the [experimental token-reduction radar](research/experimental-token-reduction-radar.md) and [fixture-only experimental benchmark starters](docs/experimental-benchmark-fixtures.md); those lanes remain experimental/non-shipped under the later-roadmap gate until matched successful tasks, failure-rate guardrails, human-correction tracking, shifted-cost accounting, provider-measured token/cost evidence, and separate future PR gates justify any hosted API savings claim or runtime feature claim.
