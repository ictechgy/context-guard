...
context-guard experiments plan image-context-pack --json --exact-text-fallback-receipt <id> --reexpand-command "context-guard-artifact get <id> --full" --provider-boundary-ack --missed-context-note "omitted text remains retrievable before any future image pack is used"
...
| `image-context-pack` | Pxpipe-inspired dry-run planning gate for future image/context packing evaluation. | Plan-only: it emits JSON metadata but no image, replacement, file, binary artifact, OCR, model call, provider call, or proxy traffic. It requires explicit opt-in, protected-zone denial, missed-context notes, exact text fallback that must be verified before omitted text is relied on, and provider-measured matched successful tasks before hosted token/cost savings claims. `visual-crop-ocr` remains the caller-supplied visual evidence-pack surface, not a verified exact binary/image fallback. |
