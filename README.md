...
- It ships only dry-run experimental planners/checkers for learned compression (`context-guard experiments plan learned-compression`), multimodal crop/OCR metadata (`context-guard experiments plan visual-crop-ocr`), and self-hosted metrics ledger previews (`context-guard experiments plan self-hosted-metrics-ledger`); learned/synthetic compressor runtime, embeddings, rerankers, model calls, replacement generation, OCR/crop runtime, and self-hosted KV/latent inference optimization are not shipped.
...
- learned/synthetic compressor runtime, multimodal crop/OCR or visual-token pruning runtime, and self-hosted KV/latent inference optimizations. See the [experimental token-reduction radar](research/experimental-token-reduction-radar.md) and [fixture-only experimental benchmark starters](docs/experimental-benchmark-fixtures.md); these lanes require matched successful tasks, failure-rate guardrails, human-correction tracking, shifted-cost accounting, and provider-measured token/cost evidence before any hosted API savings claim. ContextGuard ships dry-run checkers/planners for learned compression and self-hosted metrics ledger previews only; it does not ship a learned compressor, replacement generator, or self-hosted KV/latent runtime optimization. The radar's later-roadmap gates also keep neural/semantic compression, trust-tiered injection-aware compression, context-diff compaction, and local proxy constraints experimental/non-shipped until a separate future PR satisfies those gates.
