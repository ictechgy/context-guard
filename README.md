...
| Experimental planners and local runtimes | Reviewing local-proxy ideas, plus explicit local context-diff, visual evidence-pack, learned-compression candidate, and self-hosted metrics runtimes. | Default-off and explicit-command-only; no compressor/model execution, generated compression, OCR/crop service, forwarding, or hosted-savings claim without separate evidence and future PR gates. |
...
Shipped experimental checker/planner surfaces, plus explicit local context-diff, visual evidence, and metrics runtimes, are intentionally narrow:
