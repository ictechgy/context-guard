...
3. Append a new `Generation` record to `GENERATIONS` in the same commit that lands the fourth reapply commit's ancestor chain. Do not edit any prior generation's record — the list is append-only, including its path sets and markers.
   ...
   - its `residual_markers` is empty — same vacuity, for the unrelated-feature preservation contract;
   - any `gate_b_markers` entry names an `owner_path` that is not one of that generation's own component paths — the absence check skips a marker whose owner path is missing from the `bless` tree, and that skip is only sound when the path set check already constrains the path.

   Note what is deliberately **not** required: markers are not required to be a superset of the previous generation's. Requiring that would retroactively re-validate older `bless` trees against markers added later, which is exactly the per-generation, non-retroactive property the record layout exists to provide. The rule above is a floor, not monotonicity.
