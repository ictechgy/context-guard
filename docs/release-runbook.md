...
The active `gen8` record re-blesses the task-memory and modular context-pack
command-manifest additions while preserving the hosted-statusline residual from
`gen7`. It carries every earlier Gate-B marker and path set. Its declared
residual edits are exactly the canonical and packaged command manifests; B1/B2
content remains absent from the bless tree and is restored only by its owned
reapplication commit.
