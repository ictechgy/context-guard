...
2. Author the new generation's four reapply commits (`bless`, `b1`, `b2`, `shared-integration`) with subjects that are globally unique across the whole `GENERATIONS` history (uniqueness is enforced mechanically: two commits sharing a subject make the proof fail with "found N" instead of silently picking one). `scripts/author_gate_b_generation.py` does this mechanically once the record and subject constants exist in `verify_gate_b_rollback.py` and the final component contents are in the working tree: it restores the previous bless tree, commits the three reapply sets, appends the fingerprint, commits the record (add `--record-extra tests/test_gate_b_rollback_proof.py` after updating its pins), and runs the proof. It does not decide what to bless and does not replace the bless-diff review in step 5.
