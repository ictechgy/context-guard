...
`status: "unavailable"` with exit code `2`; it is not reported as a failed
rollback. CI uses `fetch-depth: 0`. If a release checkout is shallow, fetch the
full history and rerun the command before publishing.
