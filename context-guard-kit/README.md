...
`experimental_registry.py plan image-context-pack`은 pxpipe-inspired plan-only dry-run planning gate입니다. explicit opt-in, verified exact text fallback, protected-zone denial, provider-measured boundary, missed-context guardrail을 기록하지만 image rendering/parsing, OCR execution, model/provider call, proxy traffic, binary image artifact storage, replacement emit은 하지 않습니다. `visual-crop-ocr`은 계속 caller-supplied visual evidence pack surface이며 verified exact binary/image fallback이 아닙니다.
