...
`experimental_registry.py plan self-hosted-metrics-ledger`는 명시적 local/model-server latency, memory, quality, energy, throughput, local-cost sidecar evidence를 미리 보는 dry-run preview이며 ledger를 쓰지 않습니다. `experimental_registry.py record self-hosted-metrics-ledger --ledger-jsonl ...`은 로컬 JSONL sidecar row 하나를 명시적으로 쓰지만, self-hosted telemetry를 hosted API token/cost savings claim으로 바꾸지 않습니다.
