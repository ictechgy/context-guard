...
`experimental_registry.py plan image-context-pack`은 pxpipe-inspired image/context packing을 위한 plan-only dry-run gate입니다. image rendering, OCR/image service, model call, proxy forwarding, binary artifact, replacement evidence를 만들지 않으며, omitted text를 쓰기 전에 exact text fallback receipt/re-expand metadata, protected-zone denial, missed-context note, provider/model 측정 matched-task boundary가 필요하다는 계획 metadata만 출력합니다. `visual-crop-ocr`은 caller-supplied visual evidence-pack surface로 남고, `image-context-pack`은 duplicate emitter나 verified exact binary/image fallback이 아닙니다.
