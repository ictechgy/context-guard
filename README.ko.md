...
| 실험 dry-run planner | learned compression, visual crop/OCR, self-hosted metrics, context-diff compaction, local proxy 후보를 런타임 출시 전에 검토합니다. | 모두 기본 비활성·자문 전용입니다. 별도 evidence gate 없이는 압축/OCR/프록시 런타임이나 hosted API 절감 주장으로 보지 않습니다. |
...
| --- | --- | --- |
| `context-diff-compaction` | diff에 대한 검토용 compaction 조언. | 정확한 receipt/re-expand handle 없이는 replacement text를 만들지 않습니다. |
