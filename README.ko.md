...
플러그인은 설치만으로 전역 훅을 자동 활성화하지 않습니다. 설정은 프로젝트 단위이며 사용자가 명시적으로 적용해야 합니다. 외부 AI 위임/offload도 설정하지 않습니다. 토큰 절감 헬퍼는 로컬에서 동작합니다. 예시는 `plugins/context-guard/examples/settings.example.json`을 참고하세요.
