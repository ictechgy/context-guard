...
플러그인을 설치하고 프로젝트에서 `/claude-token-optimizer:setup`을 실행하면, 잡음 많은 명령 출력, 대용량 파일 읽기, 반복 실패, secret-like 검색·diff 결과에 대한 프로젝트 로컬 가드레일이 적용됩니다 — 전역 설정은 건드리지 않습니다.
...
플러그인은 설치만으로 전역 훅을 자동 활성화하지 않습니다. 설정은 프로젝트 단위이며 사용자가 명시적으로 적용해야 합니다. 외부 AI 위임/offload도 설정하지 않습니다. 토큰 절감 헬퍼는 로컬에서 동작합니다. 예시는 `plugins/claude-token-optimizer/examples/settings.example.json`을 참고하세요.
...
대부분의 사용자는 `/claude-token-optimizer:setup`부터 시작하면 됩니다. 아래 명령은 로컬 테스트, 자동화, 특정 문제 진단에 유용합니다.
...
`CLAUDE_TOKEN_STATUSLINE_CTX_WARN=90`처럼 조정할 수 있습니다.
