...
플러그인 이름은 `context-guard`이지만, 리브랜딩 중인 현재 GitHub 저장소는
아직 `ictechgy/claude-token-tools`에 있습니다. 나중에 저장소가 rename되면
marketplace add slug를 `ictechgy/context-guard`로 바꿀 수 있습니다.
