...
| 예산 기반 컨텍스트 패커 | 우선순위가 있는 로컬 파일 근거를 정해진 바이트 예산 안의 Markdown pack으로 조립하고, 누락 사유와 정확 slice 명령을 남깁니다. |
...
`context-guard-pack`은 우선순위가 있는 로컬 파일 근거를 렌더링된 UTF-8 바이트 기준 `--budget-bytes` 안의 Markdown 본문으로 조립합니다. JSON 출력은 포함·부분 포함·중복·unsafe·missing·예산 초과 누락 source를 기록하고, `.context-guard/packs`에 bounded 로컬 영수증을 쓰며, 정확한 정제 slice를 다시 가져오는 명령을 제공합니다. 바이트 수는 관측값이고, 토큰 수는 추정 proxy입니다.
