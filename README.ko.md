...
| 실험 planner/runtime | local proxy 후보는 dry-run으로 검토하고, context-diff·visual evidence pack·learned-compression caller-supplied candidate·self-hosted metrics만 명시적 로컬 런타임을 지원합니다. | 모두 기본 비활성이며 explicit command가 필요합니다. 별도 evidence gate와 future PR gate 없이는 compressor/model 실행, 생성형 compression, OCR/crop service, proxy forwarding, hosted API 절감 주장으로 보지 않습니다. |
