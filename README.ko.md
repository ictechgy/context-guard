...
## AI 코딩·도구 에이전트를 위한 ContextGuard

ContextGuard는 Claude Code 플러그인으로 시작했고 지금도 그 통합을 가장 먼저 지원합니다. 같은 로컬 우선 가드레일은 다른 AI 코딩·도구 에이전트에도 적용됩니다. 헬퍼는 평범한 로컬 명령으로 실행되며, 권고형 규칙 스니펫을 에이전트 자체의 지시 파일에 설치할 수 있습니다. 교차 에이전트 설정은 먼저 dry-run으로 동작하고, 로컬 파일만 기록하며, 변경 전 백업하고, 명시적 승인이 있을 때만 적용합니다.

| 접근 방식 | 강조점 | ContextGuard와의 관계 |
| --- | --- | --- |
| Headroom 방식 압축 | 모델에 이미 선택된 텍스트를 더 줄입니다. | ContextGuard는 손실형 단방향 압축 대신 로컬·되돌릴 수 있는 아티팩트 저장과 정확한 재조회를 선호합니다. |
| Caveman 방식 brief·교차 에이전트 설치 | 여러 에이전트에 간결 출력 규칙을 설치합니다. | ContextGuard는 권고형 brief 모드 스니펫과 dry-run 교차 에이전트 설정을 제공하며, 절감을 보장한다고 주장하지 않습니다. |
