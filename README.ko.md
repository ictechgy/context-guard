...
| 안정적인 프롬프트 앞부분보다 자주 바뀌는 컨텍스트가 먼저 오는 경우 | 제한된 정제 segment hash로 프롬프트 배치를 감사해, 원문 프롬프트를 노출하지 않고 캐시 친화적이지 않은 배치 가능성을 알립니다. |
| 좁은 작업에 비해 큰 tool/MCP catalog가 들어가는 경우 | 로컬 tool catalog를 bounded top-k schema report로 순위화하고, 전체 정제 schema는 로컬 영수증으로 다시 가져오게 합니다. |
...
| 예산 기반 컨텍스트 패커 | 우선순위가 있는 로컬 파일 근거를 정해진 바이트 예산 안의 Markdown pack으로 조립하고, 누락 사유와 안전할 때의 정확 slice 명령을 남깁니다. |
...
스캐너는 누락된 가드레일, 잡음 많은 훅, 넓은 컨텍스트 경로, 여러 AI 에이전트 규칙 표면의 크거나 민감해 보이는 지시문/규칙 파일, 그리고 bulky·sensitive 경로를 AI 컨텍스트에서 제외하기 위한 로컬 추천을 보고합니다. 추천은 Claude `permissions.deny`로 나온 항목 외에는 휴리스틱/자문 성격입니다.
...
`context-guard-pack`은 우선순위가 있는 로컬 파일 근거를 렌더링된 UTF-8 바이트 기준 `--budget-bytes` 안의 Markdown 본문으로 조립합니다. JSON 출력은 포함·부분 포함·중복·unsafe·missing·예산 초과 누락 source를 기록하고, `.context-guard/packs`에 bounded 로컬 영수증을 쓰며, path/root를 안전하게 표시할 수 있을 때만 정확한 정제 slice 명령을 제공합니다. 안전하지 않으면 pack 본문과 JSON metadata에 `retrieval_omitted_reason`을 남깁니다. 바이트 수는 관측값이고, 토큰 수는 provider가 실제 측정한 절감값이 아니라 추정 `chars_div_4` proxy입니다.
...
`context-guard-tool-prune`은 로컬 tool 또는 MCP catalog를 결정적 lexical heuristic으로 순위화해 bounded top-k 자문 리포트를 만듭니다. inline schema는 관측된 UTF-8 byte budget을 지키고, 누락되거나 예산 때문에 생략된 schema는 `.context-guard/tool-prune`의 compact receipt와 별도 정제 payload로 다시 가져올 수 있습니다. 이 기능은 자문용이며 MCP 설정을 변경하지 않습니다. token 값은 provider가 측정한 절감 수치가 아니라 추정 proxy입니다.
