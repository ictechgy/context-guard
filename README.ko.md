...
- learned compression은 `context-guard experiments plan learned-compression` dry-run 안전성 checker만 제공하며, self-hosted metrics는 `context-guard experiments plan self-hosted-metrics-ledger` dry-run ledger preview만, local proxy는 `context-guard experiments plan local-proxy` dry-run advisory plan만 제공하며 no listener/no traffic forwarding/no API-key persistence/read-only ledger boundary를 유지합니다. learned/synthetic compressor runtime·embedding·reranker·model call·replacement 생성, self-hosted KV/latent inference optimization runtime, 실제 proxy forwarding runtime, 실제 multimodal OCR/crop runtime·스크린샷 캡처·이미지 파싱·외부 OCR/이미지 서비스 호출은 제공하지 않습니다.
