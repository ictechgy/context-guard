...
def load_module(rel, name, extra_sys_path=()):
    ...
    return module
answer = read("answer.txt").strip()
if answer != "src/budget.py":
    fail("answer.txt does not name the defining file")
if "resolve_token_budget" not in read("src/budget.py"):
    fail("src/budget.py no longer defines resolve_token_budget")
ok()
