...
read("src/limits.py")
report = probe("""
module = load("src/limits.py", "s003_limits")
emit({"max_items": getattr(module, "MAX_ITEMS", None),
      "min_items": getattr(module, "MIN_ITEMS", None)})
""")
if report.get("max_items") != 10:
    fail("MAX_ITEMS is not the documented maximum 10, got " + repr(report.get("max_items")))
if report.get("min_items") != 1:
    fail("MIN_ITEMS changed")
if "10" not in read("docs/limits.md"):
    fail("docs/limits.md no longer documents the maximum")
ok()
