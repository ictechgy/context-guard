...
read("src/parser.py")
read("tests/sample-input.txt")
report = probe("""
module = load("src/parser.py", "s003_parser")
parse = getattr(module, "parse_fields", None)
if parse is None:
    emit({"missing": True})
else:
    try:
        none_result = parse(None)
        none_error = None
    except BaseException as exc:
        none_result = None
        none_error = type(exc).__name__
    sample = open("tests/sample-input.txt", encoding="utf-8").read()
    emit({"missing": False, "none_result": none_result, "none_error": none_error,
          "sample_result": parse(sample)})
""")
if report.get("missing"):
    fail("parse_fields is missing")
if report.get("none_error"):
    fail("parse_fields(None) still raises " + str(report["none_error"]))
if report.get("none_result") != {}:
    fail("parse_fields(None) must return an empty dict")
if report.get("sample_result") != {"alpha": "1", "beta": "two"}:
