...
read("src/telemetry.py")
report = probe("""
module = load("src/telemetry.py", "s003_telemetry")
build_record = getattr(module, "build_record", None)
if build_record is None:
    emit({"missing": True})
else:
    record = build_record("read_file", ["alpha", "beta", "gamma"])
    emit({"missing": False, "record": record,
...
if not isinstance(record, dict):
    fail("build_record must return a dict")
if report.get("keys") != ["count", "event"]:
    fail("record keys must be exactly event and count, got " + json.dumps(report.get("keys")))
if record.get("event") != "read_file":
