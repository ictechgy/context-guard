...
read("src/cli.py")
report = probe("""
module = load("src/cli.py", "s003_cli")
...
if parse_args is None:
    ...
    def call(argv):
        try:
            return {"value": parse_args(argv), "error": None}
        except BaseException as exc:
            return {"value": None, "error": type(exc).__name__ + ": " + str(exc)[:80]}
    emit({"missing": False, "new": call(["--new-flag", "x"]), "old": call(["--old-flag", "x"])})
""")
...
if (new.get("value") or {}).get("deprecated"):
    fail("--new-flag must not record a deprecation note")
if old.get("error"):
    fail("--old-flag stopped working: " + str(old["error"]))
if (old.get("value") or {}).get("value") != "x":
    fail("--old-flag stopped working")
if "--old-flag" not in ((old.get("value") or {}).get("deprecated") or []):
