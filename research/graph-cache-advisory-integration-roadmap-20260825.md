...
| Done | A1 (common cache pin via wrapper), A5 (write/read separation) | Landed 2026-08-26 (docs) as a documented `docs/wclass-advisory-workflow.md` recipe - a warm-then-share `CONTEXT_GUARD_GRAPH_CACHE_DIR` procedure, since both are task-authoring conventions rather than `context-guard-kit` code (roadmap §5). |
| Done | B2 (per-stage TTL) | Landed 2026-08-26 (#330): `--graph-cache-ttl-seconds` CLI override. The per-stage *policy* (short TTL for cheap, forced-fresh for expensive) is still a wrapper/task-authoring concern - this repo owns only the primitive that policy needs. |
| Deferred | C5 (digest integration) | On reflection, forcing graph-cache stats into the test-runner pass/fail digest (`trim_command_output.py`) has no clean natural integration point - that digest summarizes arbitrary command output, not context-guard-pack's own cache internals. Revisit only if a concrete consumer need appears; not attempted this round. |
| Deferred | B1 (two-tier cache: common `(task_id, sha)` + vendor `query_terms` sub-key) | `task_id` is a campaign-orchestration concept with no equivalent in `context-guard-kit` today; building it would mean inventing that concept here rather than reusing an existing one. A2's key (already SHA + seed_paths + query_terms) plus the A1 warm-then-share recipe already gets most of the benefit without it. Revisit only if a real multi-task campaign shows the two-tier split would help. |
| Done | B3 (impact-subgraph scoping) | Landed 2026-08-26 (#333): `--graph-impact-scope` adds diff impact-radius metadata (additive, not pruning - the "small validation" this row originally called for). |
| Done | C1 (coverage score), C3 (cache-bypass canary), C4 (drift canary) | Landed 2026-08-26 (docs) as `docs/wclass-advisory-workflow.md` recipes built on B3/A2-A4 - all three are wrapper/task-authoring procedures, not `context-guard-kit` code, same reasoning as A1/A5. |
