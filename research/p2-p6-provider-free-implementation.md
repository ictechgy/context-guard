...
| Root-scoped MCP | P2/P3 explicit local retrieval; P4 diagnostics/cache; P5/P6 adjuncts | `packages/context-guard-receipt/python/context_guard_receipt/mcp.py` | `packages/context-guard-receipt/tests/contract/test_g012_mcp.py`, `test_g012_mcp_cli.py`, `packages/context-guard-receipt/tests/e2e/test_g012_mcp_stdio.py`, adversarial `test_g012_mcp_capabilities.py` and `test_g012_mcp_limits.py` | Ephemeral stdio server for one explicit absolute root. Seven bounded tools add task-scoped exact context, progressive slices, content-free diagnostics/history, and an optional server-owned advisory twin. It installs no server/settings/hooks and exposes no runner, expiry administration, credentials, provider, or network. Without explicit `--state-dir` it creates no durable state. |
...
| --- | --- | --- | --- |
| P2 shadow broker | assembly, expansion, evidence packs, protection, diagnostics, MCP | Local candidate construction, exact recovery, and diagnostic primitives exist; P1-F is complete. | No supported host observer is established, no P2 preregistration/authority is frozen, and no live request mutation is authorized. P2 remains shadow/diagnostic. |
...
| P6 specialized tracks | expiry, MCP, ledger, twin, proof-carrying-context verifier, semantic-GC/static-relevance/image-context plan gates | Bounded local components and evaluation surfaces exist per track. The explicit MCP connects task-scoped leases/GC, process-keyed context history, shadow firewall and scout/surgeon advice, and the advisory twin. | No track inherits readiness from another. Every track remains default-off, plan/evaluation-only, or explicitly local until its own closed evidence and authority gates pass. |
...
| Provider evidence | P1-F is complete and descriptive-only; every P2-P6 phase-specific provider gate remains incomplete. |
