#!/usr/bin/env python3
"""Fail-closed v3 Anthropic live gate for the frozen 288-unit rehearsal.
...
def validate_contract(contract: dict[str, object], *, repo_root: Path) -> None:
    ...
    if top["operation"] != {
        ...
        "surface": "p3-g5-fixed-schedule-anthropic-api-v3-live",
        "version": "v3",
    }:
    ...
    if top["status"] != "approved_scope_requires_two_use_external_envelopes":
...
def _load_bound_evaluator(contract: Mapping[str, object], repo_root: Path) -> types.ModuleType:
    ...
    module_name = "contextguard_v3_captured_evaluator"
...
def _load_bound_approval_module(
    *, contract: Mapping[str, object], repo_root: Path
    ...
    module_name = "contextguard_v3_captured_external_approval"
    module = types.ModuleType(module_name)
    module.__file__ = str(path)
    sys.modules[module_name] = module
    try:
...
def _request_identity(item: Mapping[str, object]) -> str:
    ...
    return "request-" + sha256(canonical(identity))
...
def _validate_plan(plan: object) -> list[dict[str, object]]:
    ...
    for item in plan:
        ...
        for key in ("scheduled_unit_id", "task_id", "arm_id", "repetition", "prompt", "payload_sha256"):
            if key not in item:
                refuse("invalid_schedule")
        ...
        if (
            ...
            refuse("invalid_schedule")
        _prompt_from_item(item, require_identity=True)
...
def build_request_plan(
    *,
    ...
    prompt_by_unit: Mapping[str, str],
) -> list[dict[str, object]]:
    """Build plan bytes from schedule identities and bound private prompts."""
    ...
    for block in blocks:
        ...
        for unit in block["units"]:
            ...
            prompt = prompt_by_unit.get(unit_id) if isinstance(unit_id, str) else None
            if not isinstance(prompt, str):
                refuse("payload_unavailable")
            item = {
                "arm_id": unit.get("arm_id"),
                "prompt": prompt,
                "payload_sha256": sha256(prompt.encode("utf-8")),
...
def prepare_live_plan(
    *, contract: Mapping[str, object], repo_root: Path, corpus_root: Path
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="contextguard-v3-live-") as directory:
    ...
    prompt_by_unit: dict[str, str] = {}
    for binding in bindings:
        raw = prompts_by_cell.get(binding["cell_id"])
        if type(raw) is not bytes or sha256(raw) != binding["prompt_sha256"]:
            refuse("payload_identity_mismatch")
        try:
            prompt_by_unit[binding["unit_id"]] = raw.decode("utf-8", errors="strict")
        except UnicodeDecodeError:
            refuse("payload_unavailable")
    return build_request_plan(
        contract=contract,
        schedule=_schedule_object(contract, repo_root),
        prompt_by_unit=prompt_by_unit,
    )
            ...
            for item in plan:
                ...
                try:
                    ...
                    if raw != historical:
                        failed += 1
                        continue
                    with tempfile.TemporaryDirectory(prefix="contextguard-v3-score-") as directory:
...
def _write_transport_capsule(
    ...
    metadata["capsule_hmac_sha256"] = hmac.new(
        ledger_key,
        b"contextguard/p3-v3-transport-capsule/v1\0"
        + canonical(metadata),
        hashlib.sha256,
    ).hexdigest()
...
def _read_transport_capsule(
    ...
    if (
        ...
        or any(metadata[key] != identity[key] for key in (
            "arm_id", "repetition", "request_id", "scheduled_unit_id", "task_id"
        ))
    ):
        refuse("malformed_transport_capsule")
    ...
    expected_hmac = hmac.new(
        ledger_key,
        b"contextguard/p3-v3-transport-capsule/v1\0" + canonical(unsigned),
        hashlib.sha256,
    ).hexdigest()
...
def _ledger_seal(value: Mapping[str, object], key: bytes) -> str:
    return hmac.new(key, b"contextguard/p3-v3-ledger/v2\0" + canonical(value), hashlib.sha256).hexdigest()
...
def _external_scope(
    *,
    ...
    return {
        ...
        "operation": {
            "receipt_schema": receipt_schema,
            "surface_id": "p3-g5-fixed-schedule-anthropic-api-v3-live",
            "version": "v3",
        },
        "runtime": {
            "argv_sha256": sha256(
                b"contextguard/p3-v3-approval-argv/v1\0" + canonical(argv_projection)
            ),
            ...
            "identity": "python-http.client-p3-v3-live",
            "version": "v3",
        },
        "credential": {
...
def _plan_digest(batches: list[dict[str, object]]) -> str:
    ...
    return sha256(b"contextguard/p3-v3-request-plan/v1\0" + canonical(projection))
...
def _initialize_ledger(
    ...
    *,
    ...
    if state:
        if state.get("schema_version") != "contextguard.p3-v3-ledger/v2":
    ...
    for item in validated_plan:
        ...
        sealed_units.append({
            ...
            for key in (
                ...
                "scheduled_unit_id", "status", "task_id", "timing", "usage",
            )
        })
    public = {
...
def main(argv: list[str] | None = None) -> int:
    del argv
    print(
        "direct mutable Anthropic API execution is unavailable; use bound prompts and two one-use approval envelopes",
        file=sys.stderr,
    )
    return 2
