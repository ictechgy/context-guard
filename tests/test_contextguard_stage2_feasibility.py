...
EXPECTED_PRODUCTION_INVENTORY_COUNT = 152
EXPECTED_PRODUCTION_INVENTORY_SHA256 = (
...
def production_surface_inventory() -> list[dict[str, str]]:
    ...
    visible_paths = sorted({item.decode("utf-8") for item in raw_paths.split(b"\0") if item})
...
def validate_production_surface_inventory(inventory: list[dict[str, str]]) -> None:
    ...
    if len(inventory) != EXPECTED_PRODUCTION_INVENTORY_COUNT:
        raise AssertionError("production inventory path count drifted")
    if hashlib.sha256(canonical).hexdigest() != EXPECTED_PRODUCTION_INVENTORY_SHA256:
